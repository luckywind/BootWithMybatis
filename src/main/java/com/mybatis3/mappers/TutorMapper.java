...
//	@Select("select * from courses where tutor_id=#{tutorId}")
	@Select("select course_id as courseId,name, description,start_date as startDate,end_date as endDate ," +
			"tutor_id as tutor from courses where tutor_id=#{tutorId}")
	/**
	 * 注意，查询结果字段名要和Javabean属性名一致，否则相应字段的值查出来为空，
	 * 所以最好不要使用 select * ....
	 */
//	@ResultMap("com.com.mybatis3.mappers.TutorMapper.CourseResult")
	...
	@Results({
		...
		@Result(property="address", column="addr_id",
				one=@One(select="com.com.mybatis3.mappers.AddressMapper.selectAddressById")),
		@Result(property="courses", column="tutor_id",
				many=@Many(select="com.com.mybatis3.mappers.TutorMapper.selectCoursesByTutorId"))
	})
	Tutor selectTutorWithCoursesById(int tutorId);


	/**
	 * 动态SQL  不用手动在这里拼接sql，放到一个方法中使用SQL辅助类去拼SQL
	 * 使用注解SelectProvider指定提供sql的类和方法名
	 */
	...
	@ResultMap("com.com.mybatis3.mappers.TutorMapper.TutorResult")
